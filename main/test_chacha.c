/**
 * NOTE: These test functions were made by insanecoding in their C99 Chacha20 implementation. They were modified to test this chacha20 AVR drivers.
 * RIGHTS: http://chacha20.insanecoding.org/
 */
#include <stdio.h>
#include <string.h>
#include <alloca.h>
...
void test_keystream(const char *text_key, const char *text_nonce, const char *text_value, unsigned int number) {
  ...
  uint8_t key[32];
  uint8_t nonce[8];
  uint32_t value_len = strlen(text_value) / 2;
...
void test_encipherment(const char *text_key, const char *text_nonce, const char *text_plain, const char *text_cipher, uint64_t counter, unsigned int number) {
  ...
  while (i < len) {
    ...
    chacha20_set_counter(&ctx, counter);
    for (j = 0; j < len; j += i) {
      chacha20_encrypt_bytes(&ctx, plain+j, output+j, MIN(i, len-j));
    }
    if (memcmp(output, cipher, len)) {
  ...
  for (i = 0; i < 1000; ++i) {
    ...
    for (j = 0; j < len; j += amount) {
      amount = rand() & 15;
      chacha20_encrypt_bytes(&ctx, plain+j, output+j, MIN(amount, len-j));
    }
    if (memcmp(output, cipher, len)) {
